$(document).ready(() => {
    ...
    chrome.runtime.sendMessage({ name: REQUEST_DATA_FOR_PAINT, payload: window.location.href }, (results) => {
        console.log('got the response data', results)
        if (results.payload) {
            console.log('painting now');
            OVERLAY.paint(results.payload.analysis, results.payload.scrape);
        }
    });
